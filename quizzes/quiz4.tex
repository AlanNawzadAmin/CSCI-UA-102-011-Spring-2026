...
static <E> int sumHeight(Tree<E> tree) {
    ...
    return computeHelper(tree, tree.root())[1];
}

// Returns {height, sumOfHeights} for the subtree at p.
static <E> int[] computeHelper(Tree<E> tree,
                                Position<E> p) {
    int h = 0;
    int sum = 0;
    for (Position<E> c : tree.children(p)) {
        int[] result = computeHelper(tree, c);
        h = Math.max(h, 1 + result[0]);
        sum += result[1];
    }
    sum += h;                     // add this node's height
